#!/bin/bash
input_file="./examples/input/sample_TIP_testset.xlsx"
scanned_file="./examples/input/sample_TIP_test_scanned.csv"
rbv_model_num=2000
rbv_model_fold="./saved_models/RBV"
rbv_output_fold="./examples/RBV_output"
tip_model_num=2000
tip_model_fold="./saved_models/TIP"
tip_output_fold="./examples/TIP_output"
process_sum=32
...
nohup python -u ./src/predict_TIP.py -i $input_file -sf $scanned_file -tmn $tip_model_num -tmf $tip_model_fold -to $tip_output_fold -p $process_sum > $tip_output_file 2>&1 &

# monitor predicting data
echo "Submitting TIP predicting tasks."
description="Predicting"
monitor=$tip_output_fold"/monitor"
total_num=$(expr $(expr $tip_model_num / 100) \* $process_sum)
python ./src/tqdmbar.py $total_num $monitor $description
echo "Finish."
