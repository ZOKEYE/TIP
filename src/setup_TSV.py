...
from lib.log import *
from train_TSV import *
from cross_validation_draw import crossValandDraw


def tsvTask(process_num, args, PID_fold, log_fold, save_test_fold, save_pic_fold):
    ...
    checkLog(log_file)
    logger = createLogging(log_file, 'Train TSV model')
    ...
    trained_tsv_model_num = math.ceil(tsv_model_num / process_sum)
    start_model_index = process_num * trained_tsv_model_num
    stop_model_index = min([(process_num + 1) * trained_tsv_model_num, tsv_model_num])
...
def trainTSV(args):
    ...
    PID_fold = '{}/PID'.format(args.tsv_output_fold)
    if os.path.exists(PID_fold):
        removeFolds(PID_fold)
    os.mkdir(PID_fold)

    # create fold for log file fold
    log_fold = '{}/log'.format(args.tsv_output_fold)
    ...
    for p_num in range(process_sum):
        p = Process(target=tsvTask, args=(p_num, args, PID_fold, log_fold, save_test_fold, save_pic_fold, ))
        p.start()
